...
*/
function readDir(path, level, itemList, upid) {
                    ...
                    try {
                        ...
                        if (stats.isDirectory()) {
                            itemList.removeItem({ path: newPath }).then(function (msg) {
                                folderArray.push(readDir(newPath, parseInt(msg), itemList, upid));
                            }).catch(function (msg) {
                                ...
                                models.items.create({ name: data[i], parent: level, type: 0, path: newPath, upid: upid }).then(function (createditem) {
                                    ...
                                    folderArray.push(readDir(newPath, newlevel, itemList, upid));
                                });
                            });
                        } else {
                            if (checkType(utils.fixExtension(data[i]))) {
                                itemList.removeItem({ path: newPath }).catch(function (msg) {
                                    ...
                                    models.items.create({ name: tmp.name, parent: level, type: 1, path: newPath, upid: upid });
                                });
                            }
                        }
                    } catch (err) {
        ...
        init().then(function (asd) {
            fillItemList().then(function (itemList) {
                ...
                for(let i in basePath) {
                    promiseArray.push(readDir(basePath[i].path, 0, itemList, basePath[i].id));
                }
